...
export async function GET() {
  try {
    ...
    const [quotesRes, invoicesRes, submissionsRes, tasksRes, actionsRes] = await Promise.all([
      supabase
        .from('documents')
        .select('id, status, bucket, in_production')
        .eq('doc_type', 'quote')
        .neq('status', 'archived')
    ...
    // Submissions with TODO actions = action_needed
    for (const sub of submissions) {
      if (subsWithTodo.has(sub.id)) actionNeededCount++
    }

    // Quotes (not COLD) with action_needed state
    for (const doc of quotes.filter(q => q.bucket !== 'COLD')) {
      if (getCardState(doc) === 'action_needed') actionNeededCount++
    }

    // Invoices (not COLD) with action_needed state
    for (const doc of invoices.filter(i => i.bucket !== 'COLD')) {
      if (getCardState(doc) === 'action_needed') actionNeededCount++
    }
