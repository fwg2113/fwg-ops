...
export async function POST(request: Request) {
  ...
  try {
    // Create Stripe Checkout Session with ACH/Bank transfer
    const response = await fetch('https://api.stripe.com/v1/checkout/sessions', {
      ...
      body: new URLSearchParams({
        ...
        'cancel_url': `${process.env.NEXT_PUBLIC_APP_URL || 'https://fwg-ops.vercel.app'}/view/${documentId}`,
    ...
    if (session.error) {
      console.error('Stripe ACH error:', session.error)
