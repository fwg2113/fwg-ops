...
export default async function HandsPage() {
  const [tasksRes, categoriesRes, locationsRes, teamRes, prizesRes, completionsRes] = await Promise.all([
    supabase
      .from('nih_tasks')
    ...
    supabase.from('nih_task_completions').select('*').order('completed_at', { ascending: false }),
  ])
