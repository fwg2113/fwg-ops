...
// Custom sounds are stored in Supabase Storage as uploaded audio files
...
export function playBuiltinSound(key: string) {
  ...
  switch (key) {
    ...
    case 'xylophone': {
      playTone(ctx, 523, now, 0.2, 'sine', 0.25)       // C5
      playTone(ctx, 659, now + 0.12, 0.2, 'sine', 0.25) // E5
      playTone(ctx, 784, now + 0.24, 0.2, 'sine', 0.25) // G5
      playTone(ctx, 1047, now + 0.36, 0.4, 'sine', 0.3) // C6
      break
    }
    case 'radar': {
    ...
    case 'marimba': {
      ...
      playTone(ctx, 1047, now, 0.15, 'sine', 0.1) // harmonic
...
// Play a custom uploaded sound by URL
...
// Universal play function - routes to builtin or custom
