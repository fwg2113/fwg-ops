        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
          {steps.map((s, i) => (
            <div key={s.step} className="relative text-center">
              ...
              {i < steps.length - 1 && (
                <div className="hidden md:block absolute top-7 left-[60%] w-[80%] h-0.5 bg-fwg-red/30" />
              )}

              {/* Step circle */}
              <div className="w-14 h-14 mx-auto mb-4 rounded-full bg-fwg-red flex items-center justify-center text-white relative z-10">
                {ICONS[i]}
              </div>

              {/* Step number */}
              <p className="text-fwg-red-light text-xs font-semibold mb-1">
                STEP {s.step}
              </p>

              <h3 className="text-white font-bold mb-2">{s.title}</h3>
              <p className="text-fwg-text-muted text-sm leading-relaxed">
                {s.description}
              </p>
            </div>
          ))}
